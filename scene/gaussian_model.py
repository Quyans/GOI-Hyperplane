...
class GaussianModel:
    ...
    def get_semantics(self):
        if self._semantics_masks is None:
            return self._semantics
        else:
            return self._semantics * self._semantics_masks
    
    @property
    def get_opacity(self):
        return self.opacity_activation(self._opacity)
    
    def set_semantic_masks(self, masks=None):
        if masks is None:
            self._semantics_masks = None
        else:
            self._semantics_masks = masks.unsqueeze(1) # 将形状从 [N] 变为 [N, 1]
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        # sub sample
        _scale = 4
        pcd_points = np.asarray(pcd.points)[::_scale, :]
        pcd_colors = np.asarray(pcd.colors)[::_scale, :]
        ...
        rots[:, 0] = 1
        sem = torch.zeros((fused_point_cloud.shape[0], self.semantic_dim), dtype=torch.float32, device="cuda")
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device=self._opacity.device())
    ...
    def finetune_sh_setup(self, training_args):
        ...
        if training_args.position_finetune:
            ...
            optim_params.append({'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"})
            self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.position_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.position_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)
        ...
        if training_args.feature_finetune:
            ...
            optim_params.append({'params': self._features_rest, 'lr': training_args.feature_lr / 20.0, "name": "f_rest"})
        else:
    ...
    def set_opacity_through_semantic(self, mask, opacity=0.001):
        gaussian_opacity = self.get_opacity
        gaussian_opacity[mask] = opacity
        opacities_new = inverse_sigmoid(opacity)
        # self._opacity = opacities_new
        self._opacity = nn.Parameter(torch.tensor(opacities_new, dtype=torch.float).requires_grad_(True))
    ...
    def load_ply(self, path):
        ...
        sems = np.zeros((xyz.shape[0], len(sem_names) or self.semantic_dim))
        if len(sem_names) == self.semantic_dim:
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device=self._semantics.device)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_sem = self._semantics[selected_pts_mask].repeat(N,1)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_sem = self._semantics[selected_pts_mask]
